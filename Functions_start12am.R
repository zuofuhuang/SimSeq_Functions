...
simulate_one_sequence <- function(sequences, margin = 60){
  ...
  # For the following ~10 lines, we can directly sample from the first column.
  # Left this here in case it will be helpful if we will switch to starting at a random time during the day
  ...
  # Step 2
  subset <- sequences %>%
    filter(.[[starting_time]] == starting_activity)
  ...
  # Method 1: Generate activity from fitted mean/sd distribution. 
  # Don't know which distribution it follows exactly/ Didn't get truncated normal to work, so I used Poisson. Still bad.
  # fit <- fitdistr(subsequences$X3, "Poisson")$estimate
  # right <- round(rpois(1, lambda = fit))
  
  # Method 2: Draw one randomly 
