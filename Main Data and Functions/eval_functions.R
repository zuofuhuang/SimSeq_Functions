...
## Distribution of daily combined duration of each activity
...
## Distribution of each occurrence of each activity
## NEW
dist_of_occurrences <- function(traminer_seq){
  state_names <- seqstatl(traminer_seq)
  
  distinct_states <- seqdss(traminer_seq)
  durations <- seqdur(traminer_seq)
  
  output <- matrix(data = NA, nrow = 3 * length(state_names), ncol = 2)
  
  for(i in 1:length(state_names)){
    rowwise_sums <- rowSums(distinct_states == state_names[i])
    average_occurrence_time <- mean(rowwise_sums)
    dur <- durations[which(distinct_states == state_names[i], arr.ind = T)]
    occurrence_mean <- mean(dur)
    occurrence_sd <- sd(dur)
    
    output[(3*i-2):(3*i), 1] <- rep(state_names[i], 3) # first column is state name
    output[(3*i-2):(3*i), 2] <- c(average_occurrence_time, occurrence_mean, occurrence_sd)
  }
...
## NEW
dist_of_state <- function(traminer_seq, activity){
  distinct_states <- seqdss(traminer_seq)
  durations <- seqdur(traminer_seq)
  dur <- durations[which(distinct_states == activity, arr.ind = T)]
  return(dur)
}
...
## NEW
count_of_state <- function(traminer_seq, activity){
  distinct_states <- seqdss(traminer_seq)
  counts <- rowSums(distinct_states == activity)
  return(counts)
}
