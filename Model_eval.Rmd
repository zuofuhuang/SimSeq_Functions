...
For all simulated sequences, the seed is 1:n, different for each sequence.

```{r}
sequences[is.na(sequences)] <- "N/A"
# simulated_seqs_cluster <- parallel_simulate_multiple_sequences(sequences, cluster_assignment, n = 1929)
sim_seqs_clusters <- read.csv("../Data in use/sim_seqs_cluster.csv")[,-1]

# sim_seqs_noCluster <- parallel_simulate_multiple_sequences(sequences, n = 1929)
...
# sim_seqs_cluster_TVMC <- parallel_simulate_multiple_sequences(sequences, cluster_assignment, n = 1929)
sim_seqs_clusters_TVMC <- read.csv("../Data in use/sim_seqs_cluster_TVMC.csv")[,-1]


# sim_seqs_noCluster_TVMC <- parallel_simulate_multiple_sequences(sequences, rep(1, 1929), n = 1929)
...
# p1 <- seqplot(sequences_T, border = NA, type = "d", cex.legend = 0.5)
# 
# sim_seqs_clusters[sim_seqs_clusters == "N/A"] <- NA
# p2 <- seqplot(sim_seqs_clusters_T, border = NA, type = "d", cex.legend = 0.5)
# 
# sim_seqs_clusters_TVMC[sim_seqs_clusters_TVMC == "N/A"] <- NA
# p3 <- seqplot(sim_seqs_clusters_TVMC_T, border = NA, type = "d", cex.legend = 0.5)
# 
# sim_seqs_noCluster[sim_seqs_noCluster == "N/A"] <- NA
# p4 <- seqplot(sim_seqs_noCluster_T, border = NA, type = "d", cex.legend = 0.5)
# 
# sim_seqs_noCluster_TVMC[sim_seqs_noCluster_TVMC == "N/A"] <- NA
# p5 <- seqplot(sim_seqs_noCluster_TVMC_T, border = NA, type = "d", cex.legend = 0.5)
```
...
a3 <- seqmeant(sim_seqs_clusters_TVMC_T)
a4 <- seqmeant(sim_seqs_noCluster_T)
a5 <- seqmeant(sim_seqs_noCluster_TVMC_T)
...
d2 <- apply(dist2, 2, sd)
dist3 <- seqistatd(sim_seqs_clusters_TVMC_T)
c3 <- apply(dist3, 2, mean)
d3 <- apply(dist3, 2, sd)
dist4 <- seqistatd(sim_seqs_noCluster_T)
c4 <- apply(dist4, 2, mean)
d4 <- apply(dist4, 2, sd)
dist5 <- seqistatd(sim_seqs_noCluster_TVMC_T)
...
b3 <- seqlength(seqdss(sim_seqs_clusters_TVMC_T))
b4 <- seqlength(seqdss(sim_seqs_noCluster_T))
b5 <- seqlength(seqdss(sim_seqs_noCluster_TVMC_T))
```

Means: 10.3, 11.2, 10.4, 11.4, 10.4

SDs: 9.0, 7.0, 5.8, 6.8, 5.5
...
work_num3 <- table(rowSums(seqdss(sim_seqs_clusters_TVMC_T) == "WORK"))
work_num4 <- table(rowSums(seqdss(sim_seqs_noCluster_T) == "WORK"))
work_num5 <- table(rowSums(seqdss(sim_seqs_noCluster_TVMC_T) == "WORK"))
```
